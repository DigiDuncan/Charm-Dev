...
import charm.data.images.menu
from charm.lib.anim import ease_circout, perc
from charm.core.charm import CharmColors, generate_missing_texture_image
...
def load_icon_texture(icon: str, width: int, border_width: int, border_color: Color) -> Texture:
    try:
        image = img_from_path(files(charm.data.images.menu) / f"{icon}.png").resize((width, width), PIL.Image.LANCZOS)
    except Exception:
