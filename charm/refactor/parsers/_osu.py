# !: Let's get this out of the way: this is not a Parser in the prototypical sense.
# osu! charts are stored in a way that essentially requires two steps, a preprocessing step, and an interpreting step.
# What are you are seeing now is the preprocessor, but the output object (RawOsuChart) is not a playable chart.
# In the future, codifying this may be valuable, but for now, if a parser requires a staged process like this,
# I plan to create an object that creates a RawXXXChart, and then an actual Parser object will "pretend" to do the full
# file -> charts flow, using this middle step in the process.
...
from charm.refactor.generic import BPMChangeEvent
            ...
            if line.startswith("["):
                ...
                if m := re.match(RE_KV_TYPE_1, line):
                    match m.group(1):
                        ...
                        case _:
                            logger.debug(f"Unknown Metadata metadata '{m.group(1)}' (Line {line_num}).")
